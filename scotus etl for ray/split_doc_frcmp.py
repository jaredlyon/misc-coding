...
# --- CONFIG ---
input_file = "frcmp.txt"  # Path to your plain text file
output_dir = "output_md_frcmp"  # Directory to save .md files
...
for rule in rules:
    ...
    # Match rule header with or without period after rule number
    header_match = re.match(r"^Rule\s(\d+(?:\.\d+)?[A-Za-z]*)(?:\.\s|\s)(.+)", rule)
    if not header_match:
    ...
    # Compose title line with a period added between number and title
    title_line = f"Rule {rule_number}. {title_text}"

    # Filename and metadata
    filename = f"rule_{rule_number.lower()}.md"
    permalink = f"/frcmp/rule_{rule_number.lower()}/"
    title_line = f"Rule {rule_number}. {title_text}"

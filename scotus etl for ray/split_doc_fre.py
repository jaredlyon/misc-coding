...
# --- CONFIG ---
input_file = "Federal Rules of Evidence.txt"  # Path to your plain text file
output_dir = "output_md_fre"                      # Directory to save .md files
...
# --- Split rules ---
# We prepend 'Rule ' to each match so it's retained in the split
rules = re.split(r"(?=Rule\s\d+\s–)", content)
...
for rule in rules:
    ...
    permalink = f"/fre/rule_{rule_number.lower()}/"
    ...
    for line in body.splitlines():
        ...
        if re.match(r"\([a-h]\)", prefix):
            # Lowercase (a) – no indent
            indent = ""
        elif re.match(r"\(\d\)", prefix):
            # Numeric (1) – one indent
            indent = "&nbsp;&nbsp;"
